...
describe("GET /companies (with one or more params)", function () {
  ...
  test("Filters companies by 2 params", async () => {
    const query = { name: "c", minEmployees: 2 };
    const resp = await request(app).get("/companies").query(query);
    expect(resp.statusCode).toEqual(200);
    expect(resp.body.companies.length).toEqual(2);
  });
  test("Filters companies by 1 param", async () => {
...
describe("GET /companies/:handle", function () {
  ...
  test("works for anon having 1 job", async function () {
  ...
  test("works for anon having 0 jobs", async function () {
